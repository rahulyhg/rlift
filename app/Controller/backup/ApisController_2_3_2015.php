...
class ApisController extends AppController {
	...
	public function test() {
	 ...
	 public function index() {
			 ...
			 $customer_id = $_POST['customer_id'];
			    $otp_check = $this->Customer->find('count', array('conditions' => array('Customer.id' => $customer_id,'Customer.otp' => $otp)));				 
				 if($otp_check != 0)
				 {
					...
					{
						$genpromo = Array('PromoCode' => Array('promo_code' => $promo_code,'amount' => '50','promo_type'=>'signup offer'));
						if($this->PromoCode->save($genpromo))
						{
							$promos = Array('PromoUse' => Array('promo_code' => $promo_code,'customer_id' => $customer_id,'amount' => '50'));
       ...
       //  $refcode = $_POST['refcode'];
                ...
                $otp =  substr(number_format(time() * rand(),0,'',''),0,4);
				$promo_code =  substr(number_format(time() * rand(),0,'',''),0,2);
	...
	Configure::write('debug',0);
		...
		{ 	
			...
			$status = '[{"status":"success"},{"ride_id":"'.$rideid.'"},{"promocode":"'.$promocode.'"},{"amount":"'.$promoamt.'"}]';
		    echo $status;
		}
		else
		{
