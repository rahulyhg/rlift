...
class ApisController extends AppController {
	...
	var $uses = array('Customer','User','VehicleName','Country','State','City','Price','AirportPrice','PromoCode','Driver','InfoVehicle','Location','DriverLocation','Notification','Ride');
	  
	function beforeFilter()
    {
       $this->Auth->allow('signup','signin','editprofile','resetpass','emergency','location','driver_location','driver_signin','getdrivers','driver_logout','driver_notification','get_notification','getpromos','notiview','noti_action','noti_check','otp','cities','driver_details','noti_details','veh_details','rides','price_details','test','index');
	}
	...
	public function getdrivers() {
				   ...
				   if(!empty($_POST)) {
					   ...
					   $status = $this->Driver->query('SELECT lat,lon,id,fname,lname,email_id,phone,driver_photo,isverified,isactive,Isallocated,action,((ACOS(SIN('.$lat.' * PI() / 180) * SIN(lat * PI() / 180) + COS('.$lat.' * PI() / 180) * COS(lat * PI() / 180) * COS(('.$lon.' - lon) * PI() / 180)) * 180 / PI()) * 60 * 1.1515) AS `distance` FROM `ritecab_drivers` WHERE isactive = "yes" and action = "Active" and Isallocated = "allocated" HAVING `distance` <= 5 ORDER BY `distance` ASC');
				   ...
				   if(!empty($_POST)) {
					  ...
					  if(empty($cabtype))
					   ...
					   foreach($status as $key=>$value)
					   {
						   ...
						   // for checking driver is logged in or not
						   if($isactive == 'yes' && $stop == '0' && $cab_type == $cabtype && $action == 'Active')
						   {
							   $driver_id = $value['ritecab_drivers']['id'];
							   $stop = 1;
						   }
	    ...
	    public function get_notification() {
			...
			Configure::write('debug',0);
				   if(!empty($_POST)) {
					   $driver_id = $_POST['driver_id'];
					   $status = $this->Notification->find('all', array('conditions' => array('Notification.driver_id' => $driver_id, 'Notification.Issen' => 'no'),'fields'=>array('Notification.id')));	
