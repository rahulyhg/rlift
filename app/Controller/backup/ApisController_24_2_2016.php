...
class ApisController extends AppController {
	...
	public function test() {
	 ...
	 public function promosave() {
			 ...
			 $customer_id = $_POST['customer_id'];
					...
					$this->request->data['Loginpromotion']['customer_id'] = $customer_id;
					$this->request->data['Loginpromotion']['promo_code'] = $promo_code;
					if($this->Loginpromotion->save($this->request->data))
					{
			    ...
			    $otp_check = $this->Customer->find('count', array('conditions' => array('Customer.id' => $customer_id,'Customer.otp' => $otp)));				 
				 if($otp_check != 0)
				 {
					...
					if($this->Customer->save($this->request->data))
					{
						...
						$this->request->data['LoginPromotion']['customer_id'] = $customer_id;
						$this->request->data['LoginPromotion']['promo_code'] = $promo_code;
						$this->LoginPromotion->save($this->request->data); 
	    ...
	    // call for resend otp
	     ...
	     Configure::write('debug',0);
			 ...
			 if(!empty($_POST)){ 
				...
				$otp = file_get_contents("http://alerts.valueleaf.com/api/v3/index.php?method=sms&api_key=A627cf6ba484176d4d4c0f67897396765&to=".$phone_no."&sender=VLDEMO&message=Dear%20".$name.",%20your%20verification%20code%20is%20".$otp.".Team%20RiteCab&format=php&custom=1,2&flash=0");
	   ...
	   die();
		 ...
		 if(!empty($_POST)) {
			 ...
			 file_get_contents("http://alerts.valueleaf.com/api/v3/index.php?method=sms&api_key=A627cf6ba484176d4d4c0f67897396765&to=".$phone_no."&sender=VLDEMO&message= Your RiteLift new password is ".$pwd.". Team%20RiteLift&format=php&custom=1,2&flash=0");
							...
							if($this->Notification->save($this->request->data))
							 { 
								...
								if($status == 'accept')
								{
			                         ...
			                         file_get_contents("http://alerts.valueleaf.com/api/v3/index.php?method=sms&api_key=A627cf6ba484176d4d4c0f67897396765&to=".$phone_no."&sender=VLDEMO&message= Your RiteLift cab is booked. Your driver name is".$fname." ".$lname." (".$driverid."). He is riding ".$veh_color." ".$veh_name." (".$veh_number.").%20Team%20RiteLift&format=php&custom=1,2&flash=0");
	...
	Configure::write('debug',0);
		...
		{ 	
			file_get_contents("http://alerts.valueleaf.com/api/v3/index.php?method=sms&api_key=A627cf6ba484176d4d4c0f67897396765&to=".$phone_no."&sender=VLDEMO&message=Dear%20".$name.",Thanks for travelling with RiteLift. Total cost is Rs. ".$total_price." for  your ride ".$ride_id.".%20Team%20RiteLift&format=php&custom=1,2&flash=0");
			$this->Driver->updateAll(array('Driver.have_noti' => "'no'"),array('Driver.id' => $driver_id));
			$status = '[{"status":"success"}]';
		    echo $status;
				   ...
				   if(!empty($_POST)) {
					   ...
					   $whocancel = $_POST['whocancel'];
						...
						{ 
							...
							file_get_contents("http://alerts.valueleaf.com/api/v3/index.php?method=sms&api_key=A627cf6ba484176d4d4c0f67897396765&to=".$phone_no."&sender=VLDEMO&message=Dear%20".$name.",%20Sorry!%20your%20trip%20has%20been%cancelled.%20Team%20RiteLift&format=php&custom=1,2&flash=0");
	...
	public function book_seat() {
		Configure::write('debug',0);
			...
			$result = $this->Customer->query($query);
				...
				$email = $result[0]['ritecab_customers']['email'];
				$phone_no = $result[0]['ritecab_customers']['phone_no'];
		$otp = file_get_contents("http://alerts.valueleaf.com/api/v3/index.php?method=sms&api_key=A627cf6ba484176d4d4c0f67897396765&to=".$phone_no."&sender=VLDEMO&message=Dear%20".$name.",%20your%20seat%20has%20been%20booked%20successfully.%20Team%20RiteCab&format=php&custom=1,2&flash=0");
  ...
  if(!empty($_POST['pickup_city']) && !empty($_POST['drop_city']) && !empty($_POST['dep_date']) && !empty($_POST['dep_time']) && !empty($_POST['customer_id'])){			
		...
		if($this->IntercityOwnCabOrder->save($data)){
			...
			$result = $this->Customer->query($query);
				$name = $result[0]['ritecab_customers']['name'];
				$phone_no = $result[0]['ritecab_customers']['phone_no'];
		$otp = file_get_contents("http://alerts.valueleaf.com/api/v3/index.php?method=sms&api_key=A627cf6ba484176d4d4c0f67897396765&to=".$phone_no."&sender=VLDEMO&message=Dear%20".$name.",%20your%20own%20cab%20booked%20successfully.%20Team%20RiteCab&format=php&custom=1,2&flash=0");
