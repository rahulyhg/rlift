	 ...
	 public function index() {
	   ...
	   // echo $homepage;
		...
		public function promo_confirm() {
		 ...
		 if($promo_code != 'RL50')
		 {
			 $promovalid = $this->PromoCode->find('first', array('conditions' => array('PromoCode.to_time <=' => $tdate, 'PromoCode.from_time >=' => $tdate, 'PromoCode.isactive' => 'yes', 'PromoCode.promo_code' => $promo_code)));				 
             // print_r($promovalid);
              if(empty($promovalid))
			  {
					 $status = '[{"status":"sorry!! promocode you entered is wrong or get expire."}]';
					 echo $status;
					 exit;
			  }
			  else
			  {
				 ...
				 $promos = Array('PromoUse' => Array('promo_code' => $promo_code,'customer_id' => $customer_id,'amount' => $amount,'isactive' => 'yes'));
		 ...
		 else
		 {
		        ...
		        $promocheck = $this->PromoUse->find('count', array('conditions' => array('PromoUse.promo_code' => $promo_code, 'PromoUse.customer_id' => $customer_id, 'PromoUse.isactive' => 'no', 'PromoUse.is_used' => 'no')));				 
				 ...
				 {
					  $this->PromoUse->updateAll(array('PromoUse.isactive' => '"yes"'),array('PromoUse.promo_code' => $promo_code)); 
					  $status = '[{"status":"success"}]';
					  echo $status;
					  exit;
				 }	 
         ...
         $repassword = $_POST['repassword'];	
				...
				{ 
					 ...
					 $status['0']['code'] = '200'; 
							...
							$promos = Array('PromoUse' => Array('promo_code' => $promo_code,'customer_id' => $customer_id,'amount' => '50','cal_type' => 'Rs'));
							
							if($this->PromoUse->save($promos))
						    {
						       echo json_encode($status);  
							   $asp = "http://alerts.valueleaf.com/api/v3/index.php?method=sms&api_key=Acc532bf1ae4fbf9277ec150f17bf3b75&to=".$phone_no."&sender=RLIFTT&message=Hello,%20Please%20use%20this%20OTP%20".$otp."%20to%20authenticate%20your%20request.%20Use%20".$promo_code."%20to%20avail%20Rs.%2050%20off%20on%20your%20first%20ride.%20Thank%20you%20for%20registering%20with%20us.&format=php&custom=1,2&flash=0";
					           $send = file_get_contents($asp);
							   exit;
						    }	
	...
	Configure::write('debug',0);
		...
		{ 	
			...
			if($promo_code != 'false')
			{
				if($caltype == 'Rs')
			    {   
				   $sms = "http://alerts.valueleaf.com/api/v3/index.php?method=sms&api_key=Acc532bf1ae4fbf9277ec150f17bf3b75&to=".$phone_no."&sender=RLIFTT&message=Please%20pay%20Rs.%20".$total_price."%20to%20the%20driver.%20Rs.".$amount."%20discount%20applied.%20E-receipt%20has%20been%20sent%20to%20your%20registered%20Email%20ID.%20Thank%20you%20for%20choosing%20rLIFT.&format=php&custom=1,2&flash=0";
			    }
				else
				{
				   $sms = "http://alerts.valueleaf.com/api/v3/index.php?method=sms&api_key=Acc532bf1ae4fbf9277ec150f17bf3b75&to=".$phone_no."&sender=RLIFTT&message=Please%20pay%20Rs.%20".$total_price."%20to%20the%20driver.%20".$amount."%%20discount%20applied.%20E-receipt%20has%20been%20sent%20to%20your%20registered%20Email%20ID.%20Thank%20you%20for%20choosing%20rLIFT.&format=php&custom=1,2&flash=0";
			    }
			}
			else
			{
				
			    $sms = "http://alerts.valueleaf.com/api/v3/index.php?method=sms&api_key=Acc532bf1ae4fbf9277ec150f17bf3b75&to=".$phone_no."&sender=RLIFTT&message=Please%20pay%20Rs.%20".$total_price."%20to%20the%20driver.%20No%20discount%20applied.%20E-receipt%20has%20been%20sent%20to%20your%20registered%20Email%20ID.%20Thank%20you%20for%20choosing%20rLIFT.&format=php&custom=1,2&flash=0";
			}	
			
			 $send = file_get_contents($sms);
