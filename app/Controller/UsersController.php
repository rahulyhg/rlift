...
class UsersController extends AppController {
	...
	var $uses = array('User','VehicleName','Country','State','City','Price','AirportPrice','PromoCode','Driver','InfoVehicle','DriverLocation','Customer','Ride','PickupPoint','AssignVehicleToDriver','IntercityFindCabOrders','CancelTrip','CancelRide','Ride','IntercityOwnCabOrder','CustomerReciept','LaterRide');
	...
	public function dashboard() {
		...
		$laterride = $this->LaterRide->find('count');
	...
	// Later rides details
	public function later_rides() {
		
		Configure::write('debug',0);
		$cabs = $this->LaterRide->find('all'); 
 
			foreach($cabs as $key=>$val) {
			$vehicle_id = $val['LaterRide']['vehicle_id'];	
			$driver_id = $val['LaterRide']['driver_id'];
			$customer_id = $val['LaterRide']['customer_id'];
	...
	// Assign vehicles to driver and cab for later ride
	public function assign_cab_later_ride($id=null) {
		  Configure::write('debug',0);
		   ...
		   if($this->request->data) {
			 ...
			 $this->LaterRide->updateAll(array('LaterRide.vehicle_id' => "'$vehicle_id'",'LaterRide.driver_id' => "'$driver_id'"),array('LaterRide.id' => $id));
             $this->Driver->updateAll(array('Driver.Isallocated' => "'allocated'",'Driver.vehicle_id' => "'$vehicle_id'",'Driver.veh_assign' => "'$vehtype'"),array('Driver.id' => $driver_id));
			 $this->InfoVehicle->updateAll(array('InfoVehicle.Isassigned' => "'assigned'"),array('InfoVehicle.id' => $vehicle_id));
			 $this->redirect('/ride_later');  
		   }
		   else
		   {
			   if($id == null)
		       {
			     $this->redirect('/ride_later');  
				  ...
				  $assignveh = $this->LaterRide->findAllById($id,array('pickup_city','drop_city','dep_date','dep_time'));
				  $pickup_city = $assignveh['0']['LaterRide']['pickup_city'];
				  $drop_city = $assignveh['0']['LaterRide']['drop_city'];
				  $dep_date = $assignveh['0']['LaterRide']['dep_date'];
				  $dep_time = $assignveh['0']['LaterRide']['dep_time'];
				  
				  $this->set('pickup_city',$pickup_city);
				  $this->set('drop_city',$drop_city);
				  $this->set('dep_date',$dep_date);
				  $this->set('dep_time',$dep_time);
		   }	   
