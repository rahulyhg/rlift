...
class ApisController extends AppController {
					  ...
					  if(empty($cabtype))
					   ...
					   if(empty($driver_id))
					   {
						  ...
						  exit;
				            ...
				            $url = 'http://maps.googleapis.com/maps/api/geocode/json?latlng='.$lat.','.$lon.'&key=AIzaSyAugqxWYdXhQA7rGXaOTbXE3lZfs9BqNEk';
	...
	// Get vehicle details
	 ...
	 // ride later verification
	 public function ridelater_verify() {
		Configure::write('debug',0);
			if(!empty($_POST['access_code']) && !empty($_POST['driver_id']) ){			
				 $access_code = $_POST['access_code'];
				 $driver_id = $_POST['driver_id'];
				 
				   $codecheck = $this->LaterRide->find('first', array('conditions' => array('LaterRide.access_code' => $access_code, 'LaterRide.driver_id' => $driver_id)));				 
				  if(!empty($codecheck)) { 
				    die('vnvbnbvnbvn');
					  $customer_id = $codecheck['0']['LaterRide']['customer_id'];
					  $cdata = $this->Customer->find('all', array('conditions' => array('Customer.id' => $customer_id)));
					  $cdata['0']['status'] = 'success'; 
					  $cdata['0']['code'] = '200';
					  echo json_encode($cdata);
					  ...
					  $status = '[{"status":"your access code is not correct"}]';
					echo $status; 
				  }
